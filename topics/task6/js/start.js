...
function go_all_stuff(){
  ...
  // microphone variables for tasks 2 and 3
  let audioContext;
  let analyser;
  let microphone;
  let frequencyData;
  ...
  drawingBoardC.addObj(
    new FreeStyleObj(0, 100, 500, "#005A7B", "#9FBFFF", drawingBoardC.context)
  );
  ...
  /* microphone setup for task 2 and task 3 */
  navigator.mediaDevices
    ...
    .then(function (stream) {
      ...
      // turn the live mic into an audio source
      microphone = audioContext.createMediaStreamSource(stream);

      // create analyser like in the lesson
  ...
  function animationLoop() {
    ...
    // if the analyser exists, get the microphone values
    if (analyser) {
      ...
      // pass the mic average into the rectangle on board B and C
      drawingBoardB.objectsOnCanvas[0].micLevel = average / 10;
      drawingBoardC.objectsOnCanvas[0].micLevel = average / 10;
    }
